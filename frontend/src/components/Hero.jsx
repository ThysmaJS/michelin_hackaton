...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      ref={heroRef}
      style={{ position: 'relative', minHeight: 'calc(100vh - 72px)', display: 'flex', alignItems: 'center', overflow: 'hidden', background: hero.bg, transition: 'background .5s ease' }}
    >
      ...
      <HeroStage hero={hero} tiltRef={tiltRef} />

      <div style={{ position: 'relative', maxWidth: 1280, width: '100%', margin: '0 auto', padding: '0 32px', zIndex: 2 }}>
        <div style={{ maxWidth: 660, animation: 'fadeSlide .9s ease both' }}>
          ...
          <h1 style={{ margin: 0, fontSize: 'clamp(44px,6vw,82px)', lineHeight: 0.98, fontWeight: 900, letterSpacing: '-.025em', color: hero.ink, textWrap: 'balance' }}>
            Chaque route mérite<br />le <span style={{ color: hero.accent }}>bon pneu</span>.
          </h1>
          <p style={{ margin: '28px 0 0', maxWidth: 500, fontSize: 19, lineHeight: 1.6, color: hero.body }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginTop: 40 }}>
            <Hoverable
              ...
              style={{ background: '#FCE500', color: '#00205B', border: 0, fontFamily: 'inherit', fontWeight: 800, fontSize: 16, padding: '17px 32px', borderRadius: 999, cursor: 'pointer', boxShadow: '0 10px 30px rgba(252,229,0,.32)', transition: 'transform .2s,box-shadow .2s' }}
            ...
            <Hoverable
              ...
              style={{ background: hero.secBg, color: hero.secInk, border: `1px solid ${hero.secBorder}`, fontFamily: 'inherit', fontWeight: 700, fontSize: 16, padding: '17px 30px', borderRadius: 999, cursor: 'pointer', backdropFilter: 'blur(6px)', transition: 'background .2s,transform .2s' }}
          ...
          <div style={{ display: 'flex', gap: 36, marginTop: 54 }}>
            {heroStats.map((s) => (
              <div key={s.label}>
                <div style={{ fontSize: 30, fontWeight: 900, color: hero.statNum, letterSpacing: '-.02em' }}>{s.num}</div>
